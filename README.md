...
> **v2 升级（融合 Loop Engineering）**：本 skill 原本实现的是 Harness Engineering 层（单轮运行时基础设施 + 流程纪律）。2026 年 6 月兴起的 Loop Engineering（循环工程）是建在 Harness 之上的顶层编排层，聚焦"Context + Lifecycle"。v2 将 Loop 的思维与护栏**融入设计时、沉淀为运行时约束**，而非新建 loop 文档。新增：三问筛子（是否值得循环化）、STATE.md（跨循环状态）、3d 终止/回退护栏、验证阶梯、Maker-Checker 独立审查、Improvement Loop 改进信号回填。详见 [SKILL.md](SKILL.md) 1.2/1.3/3d/4.4 节。
...
1. 下载本仓库为zip文件，并重命名为`solo-as-team.zip`
2. 打开 Trae 客户端，有Trae IDE和Trae Work两种客户端
3. 进入关于 `技能` 的设置入口，两种客户端在不同位置，可以多找找
4. Trae IDE是创建技能，Trae Work是上传技能
5. 导入`solo-as-team.zip` 文件

**Trae IDE下的操作：**
![Trae IDE下的操作](https://raw.githubusercontent.com/HuangYuan1024/solo-as-team/master/image/TraeIDE-operation.png)

**Trae Work下的操作：**
![Trae Work下的操作](https://raw.githubusercontent.com/HuangYuan1024/solo-as-team/master/image/TraeWork-operation.png)
...
### 循环工程护栏（v2 新增，来自 Loop Engineering）

- **三问筛子**：启动前判断任务是否值得循环化（每周重复 + 可验证 + 结果改变下轮），全否则不套重流程
- **STATE.md 跨循环状态**：每步先读后写，回答"当前在做什么/上次结果/等人工"，对抗 Agent 遗忘
- **终止与回退护栏（3d）**：四类终止条件 + 五命名状态 + 四回退策略（Ratchet/Rollback/Retry/Branch）+ 预算上限，防烧钱死循环
- **验证阶梯**：L1 退出码 -> L2 lint -> L3 测试 -> L4 子 Agent 抽查 -> L5 人工，底层不过不进上层
- **Maker-Checker**：审查由独立子 Agent 执行，写代码的 Agent 不评判自己的工作
- **三类信息标注**：事实/推断/不确定，防把推断写成事实、误把 AI 自信当事实（对抗认知投降）
- **Improvement Loop**：把本轮历史结果变成下一轮改进信号，回填审查清单与 skill，让系统越跑越准
